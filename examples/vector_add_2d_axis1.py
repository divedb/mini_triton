...
def add_kernel_axis1(ctx, x, y, out, n):
...
def main() -> None:
    ...
    compile_result = add_kernel_axis1.compile_cached(
    ...
    loaded = load_kernel(compile_result.ptx_path, "add_kernel_axis1")
    ...
    try:
        ...
        # Use axis-1 indexing by mapping one index per y-dimension block.
        launch_kernel(
            loaded,
            arg_specs={
                "x": buffer("float32"),
    ...
    if max_abs_error > 1e-5:
        raise RuntimeError(f"axis1 vector add verification failed with max_abs_error={max_abs_error}")

    print(f"n={n}")
    print(f"cache_hit={compile_result.cache_hit}")
    print(f"ptx_path={compile_result.ptx_path}")
    print(f"max_abs_error={max_abs_error}")
    print(f"result_head={list(host_out[:5])}")
    print(f"result_tail={list(host_out[-5:])}")
