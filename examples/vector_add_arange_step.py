...
def add_kernel_arange_step(ctx, x, y, out, n):
...
def main() -> None:
    ...
    y_values = list(range(1000, 1000 + n))

    compile_result = add_kernel_arange_step.compile_cached(
    ...
    loaded = load_kernel(compile_result.ptx_path, "add_kernel_arange_step")
    ...
    host_out_init = array("f", [0.0 for _ in range(n)])

    expected = array("f", host_out_init)
    for index in range(0, n, 2):
        expected[index] = host_x[index] + host_y[index]
    ...
    try:
        ...
        driver.memcpy_htod(device_out, host_out_init.tobytes())
        ...
        launch_kernel(
            ...
            launch_config=LaunchConfig.for_num_elements(n, add_kernel_arange_step.block_size),
        )
        driver.synchronize()
    ...
    if max_abs_error > 1e-5:
        raise RuntimeError(f"vector add (arange step) verification failed with max_abs_error={max_abs_error}")
    ...
    print(f"result_even_head={result[:10:2]}")
    print(f"result_odd_head={result[1:10:2]}")
