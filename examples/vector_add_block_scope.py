...
def add_kernel_block_scope(ctx, x, y, out, n):
    idx = ctx.arange(0, 128, 1, scope="block")
    active = idx < n
...
def main() -> None:
    ...
    compile_result = add_kernel_block_scope.compile_cached(
    ...
    loaded = load_kernel(compile_result.ptx_path, "add_kernel_block_scope")
    ...
    try:
        ...
        launch_kernel(
            ...
            launch_config=LaunchConfig.for_num_elements(n, add_kernel_block_scope.block_size),
        )
        driver.synchronize()
    ...
    if max_abs_error > 1e-5:
        raise RuntimeError(f"vector add (block scope) verification failed with max_abs_error={max_abs_error}")
