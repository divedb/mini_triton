...
def add_kernel_arange(ctx, x, y, out, n):
...
def main() -> None:
    ...
    # Index 0 is intentionally kept as a sentinel because this kernel writes from idx=1.
    x_values = [0] + list(range(1, 1001))
    y_values = [0] + list(range(1000, 2000))

    compile_result = add_kernel_arange.compile_cached(
    ...
    loaded = load_kernel(compile_result.ptx_path, "add_kernel_arange")
    ...
    try:
        ...
        launch_kernel(
            ...
            launch_config=LaunchConfig.for_num_elements(n, add_kernel_arange.block_size),
        )
        driver.synchronize()
    ...
    if max_abs_error > 1e-5:
        raise RuntimeError(f"vector add (arange) verification failed with max_abs_error={max_abs_error}")
    ...
    print(f"result_head={result[1:6]}")
